...
(function($) {    
    ...
    function drawLines(sctx, options) {
        ...
        for (var i=0; i < options.yValues.length; i++) {
					inputData.push([0, options.yValues[i]]);
				}
				var gridData = this.renderer.makeGridData.call(this, inputData);
        sctx.save();
        
				for (var i=0; i < inputData.length; i++) {
					sctx.moveTo(0, gridData[i][1]);
					sctx.lineTo(sctx.canvas.width, gridData[i][1]);
					if (options.showLabel == true) {
						var labelCoordinates = determineLabelCoordinates(sctx, options.labelLocation, options.yValues[i]);
						sctx.font = options.font;
						sctx.fillStyle = options.color;
						sctx.fillText(options.yValues[i], labelCoordinates[0], gridData[i][1] + labelCoordinates[1]);
					}
					
				}
        sctx.strokeStyle = options.color;
				sctx.stroke();
        
				sctx.restore();
    }

    function determineLabelCoordinates(context, location, string) {
			var result = [];
